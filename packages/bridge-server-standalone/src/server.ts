...
export class BridgeServer {
  ...
  private mcpSocket: ServerSocket | null = null;  // MCP Server socket
  ...
  async listen(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Note: socket.io Server by default binds to 0.0.0.0 when given a port
      // The host parameter is mainly for display purposes and specific use cases
      ...
      this.io.use((socket, next) => {
        // Allow MCP Server to connect even if Chrome extension is connected
      ...
      this.io.on('connection', (socket) => {
        ...
        try {
          // Distinguish between Chrome extension and MCP Server
          const clientType = socket.handshake.query.client_type;
          ...
          socket.on(BridgeEvent.CallResponse, (params: BridgeCallResponse) => {
            ...
            // If this response is for a local call, trigger callback
          ...
          // Listen for Call events from MCP Server and forward to Chrome Extension
          if (isMcpClient) {
          ...
          socket.on('disconnect', (reason: string) => {
            // Only close server if Chrome extension disconnects
            const clientType = socket.handshake.query.client_type;
            const isMcpClient = clientType === 'mcp_server';
